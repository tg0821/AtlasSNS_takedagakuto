...
<!-- 上の投稿ボタンを押した後の投稿内容を見れるようにする -->
 <h2>投稿一覧</h2>
    ...
    @else
        @foreach($posts as $post)
            ...
            <button class="button js-modal-button" data-id="{{ $post->id }}" data-text="{{ $post->post }}">
               <img src="{{asset('images/edit_h.png') }}">編集
            </button>
            @endif
            <!-- 削除機能のボタン -->
    @auth
  {!! Form::open(['url' => '/post/delete', 'method' => 'POST', 'id' => 'deleteForm']) !!}
    ...
    <button type="submit" onclick="return confirm('この投稿を削除しますか？')" class="delete-button">
      <img src="{{ asset('images/trash-h.png') }}" alt="削除">
    </button>
  {!! Form::close() !!}
@endauth
...
<!-- フォローしているユーザーの投稿 -->
<h2>フォローしているユーザーの投稿</h2>
@foreach(Auth::user()->following as $user)
    <h3>{{ $user->username }}の投稿</h3>
    @foreach($user->posts as $post)
        <div class="post">
            <p>{{ $post->post }}</p> <!-- フォローしているユーザーの投稿内容 -->
            <!-- 編集ボタン (自分の投稿のみ表示) -->
            @if(Auth::id() == $post->user_id)
                <button onclick="openEditModal({{ $post->id }}, {!! json_encode($post->post) !!})">編集</button>
            @endif
        </div>
    @endforeach
@endforeach
